<script lang="ts">
    ...
    import {
        ...
        createNewGame,
        createLetterStates,
        words, getTodayUTCWhichUsedToBeASeed, numberIsZero,
    } from "../utils";
    ...
    function win() {
        ...
        if (!modeData.modes[$mode].historical) {
            ...
            if ("streak" in stats) {
                stats.streak =
                    getTodayUTCWhichUsedToBeASeed() - stats.lastGame > modeData.modes[$mode].unit
                        ? 1
                        : stats.streak + 1;
                if (stats.streak > stats.maxStreak) stats.maxStreak = stats.streak;
            }
            stats.lastGame = getTodayUTCWhichUsedToBeASeed();
    ...
    function reload() {
        ...
        word = words.words[game.wordNumber];
...
<Modal bind:visible={showStats}>
    {#if modeData.modes[$mode].historical}
        <h2 class="historical">Statistics not available for historical games (word {modeData.modes[$mode].wordNumber + numberIsZero})</h2>
    {:else}
